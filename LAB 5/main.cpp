...
int main(int argc, const char * argv[]) {
    ...
    std::cout << "In Order should be (A+B)*(C-D) or ((A+B)*(C-D)) or (((A)+(B))*((C)-(D))) \n   and is " << ptree1.inOrder() << std::endl;
    ...
    std::cout << "In Order output should be (A-B+C)*D/E or (((A-B)+C)*(D/E)) or ((((A)-(B))+(C))*((D)/(E))) \n   and is " << ptree2.inOrder() << std::endl;
    ...
    std::cout << "In Order should be A/(B+C)*(D+E) or ((A/(B+C))*(D+E)) or (((A)/((B)+(C)))*((D)+(E))) \n   and is " << ptree3.inOrder() << std::endl;
